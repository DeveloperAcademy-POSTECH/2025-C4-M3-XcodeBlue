...
struct TotalMEDBezierUIViewRepresentable: UIViewRepresentable {
    func makeUIView(context: Context) -> TotalMEDBezierView {
        let bezierView = TotalMEDBezierView(frame: CGRect(x: 0, y: 0, width: 200, height: 200))
        bezierView.backgroundColor = .clear
        return bezierView
    }
    
    func updateUIView(_ uiView: TotalMEDBezierView, context: Context) {
...
struct TodayMEDBezierUIViewRepresentable: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> TodayMEDBezierView {
        let bezierView = TodayMEDBezierView(frame: CGRect(x: 0, y: 0, width: 200, height: 200), medRate: medRate, medColor: medColor)
        bezierView.backgroundColor = .clear
        return bezierView
    }

    func updateUIView(_ uiView: TodayMEDBezierView, context: Context) {
        uiView.medRate = medRate
        uiView.medColor = medColor
        uiView.setNeedsDisplay() // 다시 그리기
    }
}
...
struct BezierView: View {
    ...
    @State var medColor: UIColor = .orange
